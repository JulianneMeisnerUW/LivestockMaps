...
t<-Sys.time()
  ...
  f1 <- resp ~ 0 + intercept + f(i, model=spde_RE1, group=i.group, control.group=list(model="ar1", hyper=list(rho=list(prior="pc.cor1", param=c(0.5, 0.8))))) + f(time, model = 'rw2', diagonal=1e-3, scale.model=TRUE, hyper=hyper) + f(cluster, model="iid", hyper=hyper) + f(survey, model='iid', hyper=hyper) + f(time.unstruct, model = 'iid', hyper=hyper) 
  
  formulae<-c(formulae, f1)
  
 f2 <- resp ~ 0 + intercept + urban + f(i, model=spde_RE1, group=i.group, control.group=list(model="ar1", hyper=list(rho=list(prior="pc.cor1", param=c(0.5, 0.8))))) + f(time, model = 'rw2', diagonal=1e-3, scale.model=TRUE, hyper=hyper) + f(cluster, model="iid", hyper=hyper) + f(survey, model='iid', hyper=hyper) + f(time.unstruct, model = 'iid', hyper=hyper) 
  
  formulae<-c(formulae, f2)
  
  f3 <- resp ~ 0 + intercept + urban + protected + f(i, model=spde_RE1, group=i.group, control.group=list(model="ar1", hyper=list(rho=list(prior="pc.cor1", param=c(0.5, 0.8))))) + f(time, model = 'rw2', diagonal=1e-3, scale.model=TRUE, hyper=hyper) + f(cluster, model="iid", hyper=hyper) + f(survey, model='iid', hyper=hyper) + f(time.unstruct, model = 'iid', hyper=hyper) 
  
  formulae<-c(formulae, f3)
  
f4 <- resp ~ 0 + intercept + urban + protected + water.body + f(i, model=spde_RE1, group=i.group, control.group=list(model="ar1", hyper=list(rho=list(prior="pc.cor1", param=c(0.5, 0.8))))) + f(time, model = 'rw2', diagonal=1e-3, scale.model=TRUE, hyper=hyper) + f(cluster, model="iid", hyper=hyper) + f(survey, model='iid', hyper=hyper) + f(time.unstruct, model = 'iid', hyper=hyper) 
  
  formulae<-c(formulae, f4)
  
f5 <- resp ~ 0 + intercept + urban + protected + water.body + elevation + f(i, model=spde_RE1, group=i.group, control.group=list(model="ar1", hyper=list(rho=list(prior="pc.cor1", param=c(0.5, 0.8))))) + f(time, model = 'rw2', diagonal=1e-3, scale.model=TRUE, hyper=hyper) + f(cluster, model="iid", hyper=hyper) + f(survey, model='iid', hyper=hyper) + f(time.unstruct, model = 'iid', hyper=hyper) 
  
  formulae<-c(formulae, f5)
  
  f6 <- resp ~ 0 + intercept + urban + protected + water.body + elevation + f(i, model=spde_RE1, group=i.group, control.group=list(model="ar1", hyper=list(rho=list(prior="pc.cor1", param=c(0.5, 0.8))))) + f(time, model = 'rw1', scale.model=TRUE, hyper=hyper) + f(cluster, model="iid", hyper=hyper) + f(survey, model='iid', hyper=hyper) + f(time.unstruct, model = 'iid', hyper=hyper) 
