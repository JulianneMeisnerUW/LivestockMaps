...
PredictFn <- function(m){
 ...
 if(country=="DRC"){
  ...
  folderData<-paste0("results/", country, "/cattle_results/models/")
  ...
  load(paste0(folderData, "cattle_results_", country.short,".Rdata"))
  ...
  for (idxP in 1:5){ #Separate model fit for 4 x 5 year periods
    ...
    for(year in years){
      ...
      message(paste0("Prediction done for year ", year, " model ", m))
      ...
      writeRaster(ras.median, paste0(directory, "CattlePrediction_", year, "_Model_", m), format="GTiff", overwrite=TRUE)
       message(paste0("Raster done for year ", year, " model ", m))
      
     }
  }
  res=list(prediction=prediction)
  message(paste0("Prediction done for model ", m))
  }else{
res = "error"
}
 return(res)
