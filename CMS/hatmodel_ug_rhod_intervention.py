...
def main():
    ...
    with Path("trajectories_ug_rhod_int_1tt_1itc.csv").open("w") as file:
...
def build_model(beta_h=0.000917341, **kwargs):
    ...
    parameters = [
        ...
        {"name":"treatment-reservoir-d",          "value":1},     # probability domestic reservoir is treated (assume lifelong infection otherwise)
    ...
    model.add_function("p-itc-d",                    "(+ (* p-feed-sd p-itc) (* p-feed-rd p-itc))")
    model.add_function("mu-v",                       "(/(-(ln (* (- 1 p-itc-d) p-survives-feed)))3) ")
