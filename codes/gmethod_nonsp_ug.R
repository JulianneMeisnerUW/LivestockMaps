...
folderData<-paste0("gform_results/Uganda/")
...
HAT<-readOGR(paste0("Data/Outcome_data/Uganda/HAT_final"), layer=paste0("HAT_finalPO_", type))
colnames(HAT@data)<-c("cluster", "year","focus","cases","density_med_c","density_sd_c", "density_q025_c","density_q975_c",   
                              ...
                              "WorldPop","LandScan","urban","nightlights","wealth","wealth_sd","wealth_q025","wealth_975",
                               "district", "travel_hrs", "protected", "parish", "conflict", "disaster",         
...
if(test=="no"){
  if(species=="cattle"){
    save(file=paste0("gform_results/Uganda/natural_course_cattle_ug_",type, ".Rdata"), validation)
  }else{
    save(file=paste0("gform_results/Uganda/natural_course_pigs_ug_",type, ".Rdata"), validation)
  }
}

if(test=="yes"){
  if(species=="cattle"){
    save(file=paste0("gform_results/Uganda/gform_cattle_mean_",type, ".Rdata"), results.mean)
    save(file=paste0("gform_results/Uganda/gform_cattle_total_",type, ".Rdata"), results.total)
    save(file=paste0("gform_results/Uganda/gform_cattle_inc_",type, ".Rdata"), results.inc)
  }else{
    save(file=paste0("gform_results/Uganda/gform_pigs_mean_",type, ".Rdata"), results.mean)
    save(file=paste0("gform_results/Uganda/gform_pigs_total_",type, ".Rdata"), results.total)
    save(file=paste0("gform_results/Uganda/gform_pigs_inc_",type, ".Rdata"), results.inc)
  }
}