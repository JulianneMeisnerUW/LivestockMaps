...
'%ni%' <- Negate('%in%')
folderData<-"Data/Exposure_data/Uganda/"

#----------------------#
#   Read in data
#----------------------#
merged_data<-readOGR(paste(folderData, "data/Created_datasets/merged_data", sep=""), layer="merged_map") 
...
proj.sp<-readOGR(paste(folderData, "data/Created_datasets/prediction_data", sep=""), layer="prediction_data")
colnames(proj.sp@data)<-c("protected", "water.body", "elevation")
proj<-proj4string(merged_data)
districts<-readOGR("Data/Exposure Data/Uganda/shapefiles/gadm36_UGA_shp", layer="gadm36_UGA_4")
ugandaMap<- districts
...
writeOGR(obj=cattle_data, dsn="Data/Exposure Data/Uganda/data/Created_datasets/cattle_data", layer="cattle_data", driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR(obj=pig_data, dsn="Data/Exposure Data/Uganda/data/Created_datasets/pig_data", layer="pig_data", driver="ESRI Shapefile", overwrite_layer=TRUE)

districts_pigs<-pig_data@data$district
districts_cattle<-cattle_data@data$district
save(file="Data/Exposure Data/Uganda/data/Created_datasets/District_vector.Rdata", districts_cattle)
save(file="Data/Exposure Data/Uganda/data/Created_datasets/District_vector_pigs.Rdata", districts_pigs)
